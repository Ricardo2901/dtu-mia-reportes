...
"""
    ========================================================================================================================
        Capitulo 1.- Generacion de documento de acuerdo al tipo de proyecto
    ========================================================================================================================
"""


def capitulo1(nombre_proyecto, carpeta_proyecto):
    tipo_proyecto = os.path.join(
        f"{nombre_proyecto.title()}"
    )
    print(f"✅ Guardado: {tipo_proyecto}")
    ...
    descripcionCapitulo11 = di11.add_run(f'Proyecto "{nombre_proyecto.title()}", que para la identificación en este documento se referirá al proyecto como "materia petreó".')
    ...
    descripcionCapitulo151 = di151.add_run(f'El proyecto objetivo del estudio que se ubica "______________", contemplado para implementar el proyecto de "{nombre_proyecto.title()}” se tiene proyectado llevar a cabo la implementación del mismo en un periodo de _____ años donde se pretende aprovechar _____ ha por año, considerando para ello 4 etapas adicionales y actividades que se desarrollarán en forma paulatina, siendo las que a continuación se describen:')
    ...
    if nombre_proyecto == 'EXTRACCION DE MATERIAL PETREO':
        ...
        tabla1 = doc.add_table(rows=len(etapas) + 1, cols=len(encabezados), style='Table Grid')
        tabla1.alignment = WD_ALIGN_PARAGRAPH.CENTER

        for i, cols in enumerate(encabezados):
            cell = tabla1.cell(0, i)
            t1 = cell.paragraphs[0].add_run(cols)
            t1.font.name = 'Arial'
            t1.font.size = Pt(12)
            t1.font.bold = True
            cell.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER  # Alineación al centro
            cell_background_color(cell, 'B8CCE4')  # Color de fondo de la celda

        for i, etapa in enumerate(etapas):
            cell = tabla1.cell(i + 1, 0)
            t1 = cell.paragraphs[0].add_run(etapa)
            t1.font.name = 'Arial'
            t1.font.size = Pt(12)
            t1.font.bold = True
            cell.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER  # Alineación al centro
        ...
        for i, actividad in enumerate(actividades):
            cell = tabla1.cell(i + 1, 1)
            t1 = cell.paragraphs[0].add_run(actividad)
            t1.font.name = 'Arial'
            t1.font.size = Pt(12)
            t1.font.bold = True
            cell.paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.CENTER  # Alineación al centro
    ...
    archivo_docx = os.path.join(
        carpeta_proyecto,
        f"CAPITULO 1 DTU {nombre_proyecto.replace(' ', '_').upper()}.docx"
    )
